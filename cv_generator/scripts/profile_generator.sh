...
sed -n '1,/\\begin{cvtable}[[:space:]]*%[[:space:]]*Profile/p' "$input_latex_file" > "$output_latex_file"

# In the range of the pattern until the end of the file
sed -n '/\\end{cvtable}[[:space:]]*%[[:space:]]*Profile/,$p' "$input_latex_file" >> "$output_latex_file"
...
sed -i '/\\begin{cvtable}[[:space:]]*%[[:space:]]*Profile/r '"$temp_file"'' "$output_latex_file"
